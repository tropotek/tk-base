...
class ModelProperty extends \Tk\Collection
{
    ...
    public function getType(): string
    {
        if (!$this->type) {
            ...
            if (preg_match('/^(tinyint\(1\)|int\(1\)|bool|boolean)/', $dtype)) {
    ...
    public function getDefinition(): string
    {
        $tpl = <<<PHP
            public %s%s $%s%s;
        PHP;
        $value = ($this->get('Null', 'NO') == 'YES') ? 'null' : $this->getDefaultValue();
        if (!is_null($value)) $value = ' = ' . $value;
        return sprintf($tpl,
            ($this->get('Null', 'NO') == 'YES') ? '?' : '',
            $this->getType(),
            $this->getName(),
            $value
        );
...
//            $tag = ', ' . $this->quote('key');
    ...
    public function getPreparedFilterQuery(): string
    {
        ...
        if ($this->getType() == self::TYPE_BOOL) {
            $filterValid = sprintf("isset(\$filter['%s']) && is_bool(\$filter['%s'])",
                $this->getName(),
                $this->getName()
            );
        }
